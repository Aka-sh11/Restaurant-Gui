...
root.title("SHARMA Restaurant ")
root.configure(background='green')

Tops = Frame(root, bg='green', bd=20, pady=5, relief=RIDGE)
Tops.pack(side=TOP)

lblTitle = Label(Tops, font=('arial', 60, 'bold'), text='SHARMA RESTAURANT ', bd=21, bg='black',
...
def Receipt():
    ...
    txtReceipt.insert(END, 'Manchurian:\t\t\t\t\t' + E_Manchurian.get() + '\n')
...
Manchurian = Checkbutton(Food_F, text="Manchurian ", variable=var13, onvalue=1, offvalue=0,
                        font=('arial', 16, 'bold'), bg='orange', command=chk_Manchurian).grid(row=4, sticky=W)
...
txtManchurian = Entry(Food_F, font=('arial', 16, 'bold'), bd=8, width=6, justify=LEFT, state=DISABLED,
                     textvariable=E_Manchurian)
txtManchurian.grid(row=4, column=1)
